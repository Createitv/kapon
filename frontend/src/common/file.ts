...
export function ep(): void {
	dataToPDF(d, '测试文件')
	console.log('test pdf done')
}

export function dataToPDF(data: Object[], filename: string) {
	const rows = Object.keys(data[0])
	let bodyData = []
	data.forEach((item) => {
		bodyData.push(Object.values(item))
	})
	console.log(data[0], bodyData)
